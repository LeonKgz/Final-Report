...
SARSA and DQNapproaches are applied, former used for
...
implemented in order to include throughput as metric into the
reward. Throughput $T$ can be calculated as follows: 
\begin{align}
T = log_2(\text{SINR}+ 1)
\end{align}
...
\texttt{energy} dimension into the state space results
in worse learning but that might also be due to 
the implementation in the simulation of information 
gathering.

\section{Reward Function}

I have tried putting different emphasis on either 
throughput or energy via altering the basic reward f
formala of $\frac{throughput}{energy}$. \\
...
Satisfying the convergence properties for both $\epslion$
